...
LUI.createMenu.MainMenu = function (LocalClientIndex)
	...
	local modText = LUI.UIText.new()
	modText:setLeftRight(true, false, 4, 300)
	modText:setTopBottom(true, false, 285, 305)
	modText:setText(Engine.Localize("Project Nebula: ^5[v2.0.3]^7"))
	modText:setFont(CoD.fonts.Default)
	modText:setAlignment(LUI.Alignment.Left)
	modText:setRGB(1, 1, 1)
	...
	MainMenuWidget:registerEventHandler("open_custom_games_lobby", CoD.MainMenu.OpenCustomGamesLobby)
	...
	MainMenuWidget:registerEventHandler("do_unlock_all", CoD.MainMenu.unlockAll)
	MainMenuWidget:registerEventHandler("open_levels_menu", CoD.MainMenu.OpenLevelsMenu)
	MainMenuWidget:registerEventHandler("open_mods_menu", CoD.MainMenu.OpenModsList)
	MainMenuWidget:registerEventHandler("open_logs_menu", CoD.MainMenu.OpenLogsMenu)
	if CoD.isPS3 then
	...
	if CoD.isPC then
		...
		MainMenuWidget.modsButton = MainMenuWidget.buttonList:addButton(Engine.Localize("MODS"), nil, 7)
		MainMenuWidget.modsButton:setActionEventName("open_mods_menu")
		MainMenuWidget.logsButton = MainMenuWidget.buttonList:addButton(Engine.Localize("CHANGELOGS"), nil, 8)
		MainMenuWidget.logsButton:setActionEventName("open_logs_menu")
		MainMenuWidget.buttonList:addSpacer(CoD.CoD9Button.Height / 2, 9)
		MainMenuWidget.quitButton = MainMenuWidget.buttonList:addButton(Engine.Localize("MENU_QUIT_CAPS"), nil, 10)
...
CoD.MainMenu.unlockAll = function(MainMenuWidget, ClientInstance)
...
CoD.MainMenu.OpenCustomGamesLobby = function (MainMenuWidget, ClientInstance)
	if CoD.MainLobby.OnlinePlayAvailable(MainMenuWidget, ClientInstance) == 1 and CoD.MainLobby.IsControllerCountValid(MainMenuWidget, ClientInstance.controller, UIExpression.DvarInt(ClientInstance.controller, "party_maxlocalplayers_privatematch")) == 1 then
		CoD.SwitchToPrivateLobby(ClientInstance.controller)
		if CoD.isZombie == true then
			Engine.SetDvar("ui_zm_mapstartlocation", "")
		else
			local PrivateOnlineLobbyMenu = MainMenuWidget:openMenu("PrivateOnlineGameLobby", ClientInstance.controller)
			Engine.Exec(ClientInstance.controller, "gts maxallocation 17")
		end
		MainMenuWidget:close()
	end
end
...
CoD.MainMenu.OpenModsList = function(MainMenuWidget, ClientInstance)
	MainMenuWidget:openMenu("Mods", ClientInstance.controller, {
		parent = "MainMenu"
	})
	MainMenuWidget:close()
end

CoD.MainMenu.OpenLevelsMenu = function(MainMenuWidget, ClientInstance)
	MainMenuWidget:openMenu("Levels", ClientInstance.controller, {
		parent = "MainMenu"
	})
	MainMenuWidget:close()
end

CoD.MainMenu.OpenLogsMenu = function(MainMenuWidget, ClientInstance)
	MainMenuWidget:openMenu("Changelogs", ClientInstance.controller, {
		parent = "MainMenu"
	})
	MainMenuWidget:close()
end
